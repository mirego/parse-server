...
var INSTALLATION_COLLECTION = '_Installation';
...
function pushStatusHandler(body, config) {
  ...
  var setInitial = function setInitial(where) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : { source: 'rest' };
  ...
  var complete = function complete(results) {
    return handler.get(objectId).then(function (object) {
      var update = {
        status: 'succeeded',
        updatedAt: new Date(),
